 ...
 * @version $Id$
 */
class Itemlog extends MQ_Controller
{
    ...
    public function index()
    {
        ...
        $params = $this->p(array(), array('page', 'pagenums', 'item_id', 'team_id', 'place_id', 'date', 'start_date', 'end_date', 'date_range', 'kw', 'ajax'), false, function () {
            show_error("页面错误");
        });
        $this->_page_title = '报表';
        ...
        $ret = $this->sys_check_log->group('item_id', 'nums', 'item_id', $params['where']);
        $total = 0;
        foreach ($ret as $v) {
