...
//这里可以捕获到前面一个块抛出的Exception
    ...
    public function v2()
    {
         ...
         */
        if (!($params = $this->_params($required, $optional))) {
            api_error(404, "param is not set: ", $required);
        }
        $model_name = $params ['model_name'];
        $this->_m($model_name);
        $this->_data ['class_name'] = $params ['class_name'];
        ...
        $data = $this->$model_name->get_cloumn();
        $out['data'] = $data;
        $out['file'] = $params['class_file'];
        $content = $this->load->view('auto/view.edit.tpl.php', $out, true);
        if (write_file(APPPATH . "/views/tpl/" . $model_name . "_edit.tpl.php", $content)) {
            echo 'ok';
        } else {
            echo 'fail';
        }
        ...
        $tmpID = $params['class_id'];
        // index start
        $content = read_file($this->config->item('root_path') . '/application/views/auto/view.index.tpl.php');

        $thead = '<th><input type="checkbox"/></th>';
        foreach ($data as $d) {
            $f = json_decode($d["COLUMN_COMMENT"]);
            if (count($f) && $f->type != "hidden") {
                $thead .= "<th><?php e($f->title);?></th>";
            }
        }
        $thead .= '<th>操作</th>';

        $tbody = '<?php foreach ($data as $k => $v): ?><tr index="0"><td><input type="checkbox" name="' . $tmpID . '" value="<?php e($v->' . $tmpID . ');?>"/></td>';
        foreach ($data as $d) {
            $f = json_decode($d["COLUMN_COMMENT"]);
            if (count($f) && $f->type != "hidden") {
                $tbody .= '<td><?php e($v->' . str_replace('_id', '_name', $d["COLUMN_NAME"]) . '); ?></td>';
            }
        }
        $tbody .= '<td>
                        <a data-id=<?php e($v->' . $tmpID . '); ?> data-keyboard="true" data-toggle="modal" data-target="#myModal">查看</a> |
                        <a href="javascript:MQC.Dialog.alertUrl(\'是否删除该' . $params['class_name'] . '\',\'' . $params ['class_file'] . '/delete?ajax=true&' . $tmpID . '=<?php e($v->' . $tmpID . '); ?>\');">删除</a>
                        </td></tr>';
        $tbody .= '<?php endforeach;?>';
        $content = str_replace('{ID}', $params['class_id'], $content);
        $content = str_replace('{NAME}', $params['class_name'], $content);
        $content = str_replace('{FILE}', $params['class_file'], $content);
        $content = str_replace('{THEAD}', $thead, $content);
        $content = str_replace('{TBODY}', $tbody, $content);
        if (write_file(APPPATH . "/views/tpl/" . $model_name . "_index.tpl.php", $content)) {
            echo 'ok';
        } else {
            echo 'fail';
        }
        // index end
        $this->_v('/auto/v2.tpl.php');
    }
}
